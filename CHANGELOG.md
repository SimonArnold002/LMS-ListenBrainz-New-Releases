...
### Fixed
- **Genres could not come back after being cleared, and that is why they stayed empty.** Clearing a genre removed the answer but left its "last checked" clock running, so every one of those albums still looked freshly checked and nothing ever asked about it again — for up to **90 days**. On a live server that left 1,033 of 1,034 albums holding no genre, with nothing attempting to repair them. An answer and its clock are now always cleared together, so anything cleared is asked about again straight away.
- **The Community API is now used to fill in gaps as you browse, not only overnight.** The background top-up that runs when a view opens with missing genres only ever asked ListenBrainz, which covers about half of a typical week; the rest waited for the nightly pass. It now works through the full ladder, so opening a view repairs what it can see.
- **Nothing is stored forever, and nothing is checked all at once.** Every genre now carries its own age: one that found something is kept for a long time, while "we asked and there was nothing" is re-checked far sooner — because MusicBrainz tagging routinely lands weeks after a release. Answers come back round gradually, spread out by when each was last looked at, instead of everything expiring together.
- **Last.fm genres are now kept in the plugin's own store.** They were the last part of the genre ladder still in the server's shared cache, where they could be evicted at any time — despite costing a deliberately slow one-request-per-second background pass to collect.

### Changed
- **Installing a build clears the caches again**, as it did before 0.9.168. That is the development rule, and gating it was the wrong fix: the harm was never the clearing, it was that cleared genres could not be re-fetched. With that fixed, a clear-out is recoverable, and it is the only way to see what a first run — or a suddenly much wider release window — actually looks like. A *released* build still only clears genres when the code that parses them changes.

### Internal
- **Schema migration 3: one column per tier, one timestamp per answer.** Three defects with one shape — a write touching something it did not own — become inexpressible rather than merely avoided:
  - a single `fetched_at` served a row holding several independent answers, so the artist-sort warm refreshing a sort-name silently declared the genres beside it freshly fetched, and could hold an empty answer alive indefinitely without ever asking about genres;
  - the hosted and MusicBrainz-mirror tiers shared `artist.genres` behind a `genres_src` discriminator, and each read the other's answer as stale — so with both live they refetched and overwrote each other on alternate passes, permanently. They now hold separate columns and cannot see each other;
  - Last.fm gets its own table, keyed as the rung actually keys it (artist + album, because it asks `album.gettoptags` before falling back to the artist).
  The step **back-fills** every existing answer into the tier that produced it, stamped with when it was really obtained rather than with "now" — without which the upgrade would silently re-age the whole store.
- **The age policy is declared once, in `DB.pm`,** so `cachestats` reports staleness from the same constants the fetchers obey. A second copy would drift and then lie, and a lying instrument is what turned a one-line bug into two bad builds. `cachestats` gains per-tier coverage and an `rg_genres_stale` figure — the number that would have shown this lockout immediately: everything empty, nothing stale, no traffic.
- `tools/t_db.pl` 218 → 238 and `tools/t_genrefill.pl` 101 → 112, with a new section in each for the interaction the old suites could not see: one proved the wipe cleared the columns, the other proved each tier fetched, and the defect lived only where they met. **Anti-tested four ways** — a write that stamps every answer (1 red), a wipe that leaves the clocks running (8 red), the pre-schema-3 shared column (3 red), and holding empty answers as long as populated ones (1 red).

## 0.9.168

### Fixed
- **Genres are no longer thrown away every time you install a build.** 0.9.166 added an automatic dev-build clear-out that wiped every stored genre on each install — so two builds in an afternoon emptied the genre store twice, and refilling it means asking about hundreds of artists again against a shared community service that is not designed to be hit in one burst. Genres now survive an ordinary build and are cleared only when the code that *parses* them actually changes. Everything else about the build clear-out is unchanged.
- **The genre coverage report was reporting on the wrong thing.** `["lbf","cachestats"]` counted only the MusicBrainz last-resort tier, so a store holding thousands of genres from the community API and ListenBrainz reported **zero** — which made a healthy store and a real regression look identical, at exactly the moment that distinction mattered. It now reports, per tier, how many artists and albums have genres, how many were asked and genuinely have none, and how many have never been looked up.

### Internal
- **The genre wipe is gated on `DB::GENRE_FACT_VERSION`, not the plugin version.** §2.3 of `docs/caching-rework.md` already specified this mechanism ("bumped when the parser changes"); `_buildChanged` was a blunter duplicate of it. The dev-build wipe still clears ALL of `kv` unconditionally — every match decision, resolved list and marker — which is what the fleet "dev builds clear caches" rule is protecting. Genres are expensive upstream fact, not a decision.
- **Schema migration 2** adds `n_genres`/`n_agenres` integer columns beside the frozen lists, because a blob cannot be counted in SQL. `-1` = never asked, `0` = asked and the answer really was none (a real answer from both ListenBrainz and the hosted API — re-asking those is the waste the store exists to stop). The step **back-fills existing rows** from the blobs they already hold, so the first report after upgrading is true rather than reading "never asked" for everything; and its `ALTER`s are individually guarded so the step stays re-runnable.
- `tools/t_db.pl` 204 → 218, including the back-fill (the live upgrade case) and the three coverage states told apart.

## 0.9.167

### Fixed
- **0.9.166 would not load at all — every feed, the menu and the settings page were empty.** A single line referred to a value in another of the plugin's own files in a form Perl resolves while the file is still being compiled, before that other file has been read. Perl treats that as a fatal error, and because it happens during loading it takes the whole plugin down rather than just that one feature. Nothing was wrong with the stored data: 0.9.167 is 0.9.166 with the line corrected, and anything 0.9.166 managed to store before it broke is still there.

### Internal
- The defect was **seen during the 0.9.166 build and dismissed**: `perl -c Plugin.pm` failed with exactly this error standing alone and passed once `DB.pm` was loaded first, which was written off as "production must load DB first". It does not, and nothing guaranteed it would. `Plugins::…::DB::KEY_VERSIONS` is now `Plugins::…::DB->KEY_VERSIONS` — a method call, resolved at runtime, so load order can never matter again. `retirePrefixes` deliberately stays a function call: it takes a plain hashref and the method form would hand it the class name instead.
- **New gate `tools/t_loads.pl`**, and it is the real fix. Every module is compiled in **its own fresh interpreter** with only LMS stubs present — the condition LMS itself imposes, and the one every other suite in `tools/` hides by loading its subject with the rest of the plugin already in memory. It also greps each file for a fully-qualified constant from another package used as a bareword, so the defect is caught at its line rather than at its symptom. Anti-tested: restoring the 0.9.166 form turns two assertions red.

## 0.9.166

### Changed
- **The release feeds are no longer thrown away and rebuilt every night.** All Releases, New Releases for You and MuSpy were each kept as one big block of data filed under today's date — so at midnight the whole thing, around 3,000 releases, expired and had to be downloaded and rebuilt from scratch. It is now kept as releases, with a record of which days have actually been fetched. Narrowing your release window (say 14 days down to 7) now costs nothing at all; widening it fetches only the days it adds; and midnight is short by a single day rather than everything.
- **A feed that has anything stored now appears instantly, and refreshes behind you.** Previously a stale feed meant waiting for ListenBrainz before anything was drawn. Now the releases are drawn immediately and the check for new ones happens behind the page. Only a completely empty store makes you wait.
- **"Refresh (force update now)" no longer empties the list while it works.** It used to delete the feed and then go and fetch it, so a slow ListenBrainz left you looking at nothing, and a failed refresh left you with nothing. It now simply marks the feed as due a check and deletes nothing, so your releases stay on screen throughout and a failed refresh costs you nothing.
- **All Releases is now prepared in the background like everything else.** The nightly preparation stopped immediately if you had no ListenBrainz username — but All Releases has never needed an account, so it had never been prepared for anyone, and the first browse of each day always waited on ListenBrainz. It is now prepared for everyone.
- **A ListenBrainz outage can no longer blank a feed.** An empty or unreadable response is treated as a failed attempt rather than as "there are no releases": nothing is deleted, and your stored releases keep being shown with their age recorded.

### Internal
- New feed layer in `DB.pm`: `BASE_VERSION`, `relId` (the identity ladder, reusing `Browse::_streamId` so the release key and the stream key can never disagree), `ingestFeed`, `feedCoverage`, `feedReleases`, `feedNoteAttempt`, `feedInvalidate`, `feedSweep`. `API.pm` gains `_feedWindow`/`_feedFromStore`/`_fetchReleaseFeed`; the `lbf:feed:*` keys and their `…fb:` twins are gone. `_cacheFeed` had no callers left and was removed.
- **MuSpy is stored with rotation OFF, and that is not a tuning choice.** Its endpoint returns a top-100 slice rather than a window, so day coverage would be a lie and window-scoped rotation would delete rows that are still valid and merely pushed past the limit — the same family as this repo's twice-learned "an empty result is never a fact". Its rows accumulate and age out on last-seen instead.
- **The dev-build wipe** (`_buildChanged`) is one unconditional `DELETE FROM kv` plus the genre columns, safe only because everything durable now has a table; releases, feed coverage, Bandcamp pins, follow items and artist sort-names all survive it deliberately. Its marker is a **pref**, never a `kv` row — the wipe would otherwise delete its own marker and every start would look like a new build.
- `tools/t_db.pl` 157 → 204 assertions (the identity ladder, coverage arithmetic, merge-not-blank, window-scoped rotation, the refuse-an-empty-ingest branch, top-N non-rotation, generation, `BASE_VERSION`, the sweep). Anti-tested four ways. `tools/t_review_fixes.pl` finding 2 follows the Refresh property down to the store and gains an `LBF_API` seam — it had none, so that finding could never have been anti-tested at all.
- Two of the plan's claims did not survive the build and are corrected in `docs/caching-rework.md`: ListenBrainz has no day-range parameter, so a coverage gap is repaired by one full fetch (what coverage buys is that it need not block); and a day's shift leaves exactly one day uncovered, not zero.
- No cache versions bumped. Nothing about a stored *decision* changed meaning — this moves where the feed lives, not what any match or resolve concluded.

## 0.9.165

### Changed
- **Genres are now on the rows the first time you open a view, instead of appearing only after you go into a release and back out.** Three things were stopping that. The nightly pre-fill only covered the first 600 releases of a feed that holds around 3,000, so most weeks were never prepared at all and filled a little at a time in the background as you browsed. It also refused to prepare your personal *New Releases for You* feed unless you had an API token, which that feed has not needed since 0.9.160. And the labels it did find were being thrown away on write (fixed in 0.9.164), which is why widening the pre-fill was never worth doing before now. It now prepares the whole feed, for everyone with a username.
- **A new genre source fills in where ListenBrainz has nothing.** Measured against the live feed on 13 August: ListenBrainz can answer for about half of it. The rest now falls through to the LMS-community metadata service, which answers per artist and covers a large share of the remainder — so albums that could never show a genre from any source now can. It also works from the artist's name alone, which means albums in the Trending lists that arrive without a MusicBrainz ID can show genres for the first time. ListenBrainz is still asked first, because its answers arrive free with a request the plugin already makes.
- **Genre look-ups no longer give up when ListenBrainz asks them to slow down.** ListenBrainz allows a burst of requests and then asks for a pause. Preparing a whole feed is well past that burst, so the pause is now expected rather than exceptional: the plugin waits for exactly as long as ListenBrainz asks and then carries on, instead of abandoning that batch of releases and leaving them blank. Everything that talks to ListenBrainz now backs off together rather than each part discovering the limit separately.

### Internal
- Hosted artist genres are stored in the plugin's own database (`artist` table, `genres_src='hosted'`), not in the Lyrion cache — so no lifetime is handed to Lyrion and the 30-day boundary that broke every other genre tier cannot apply. Staleness is an age policy: 90 days for an answer with genres, 7 for an empty one.
- The render path still never fetches, and now never reads the store per row either: the whole page's hosted genres arrive in one bulk query, folded into the render's existing genre map. The first version did one query per release (~2,900 on the genre picker's whole-feed pass) — caught by `tools/bench_walk.pl`, not by review, and pinned by a test.
- `tools/t_genrefill.pl` 55 → 101 assertions (the hosted tier's four live-verified traps, the ladder order, the whole-feed warm, the token gate, the rate-limit handling); `tools/t_db.pl` 83 → 96 (the `artist` facts tier, merge-not-blank both ways). Anti-tested five ways, all caught.
- No cache versions bumped: the ladder only ADDS a tier below the existing ones, so no stored decision changes meaning.

## 0.9.164

### Fixed
- **Genres on release rows have never actually worked, and this fixes it.** The plugin looks a release's genres up once and then remembers the answer — but it was asking to remember them for 90 days, and Lyrion treats anything longer than 30 days as a date rather than a length of time. So every genre it found was filed away as having expired in 1970, and could never be read back again. Nothing failed, nothing was logged; the labels simply never appeared, and the plugin quietly looked the same releases up again every time you browsed. Because the long remembering only applied to releases that *did* have a date, the ones worth keeping were exactly the ones thrown away. Both places this happened are fixed. **Expect noticeably more genre labels than before — that is the feature working for the first time, not something new going on.**

### Internal
- New `tools/t_ttlceiling.pl` fails any cache lifetime above Lyrion's 30-day limit, in any module, including ones written inline where a search for named constants would not find them. It reproduces Lyrion's own rule first, so a guard set to the wrong number fails before it can pass while the bug is live — the mistake that cost the sibling Pitchfork plugin five releases.
- New `ListenBrainzFreshReleases/DB.pm`: the plugin's own database, the first stage of moving storage off the Lyrion cache that lost the genre data. **Nothing reads it yet** — this release behaves exactly as the last one did. It ships now so it can be proved to survive a restart before anything depends on it, via the new `["lbf","cachestats"]` command. Guarded by `tools/t_db.pl`.
- No cache versions bumped, and it is checked rather than assumed: every entry written under the old lifetime was unreadable by construction, so there is nothing stale to clear, and bumping the match caches would re-resolve every album to learn nothing.

## 0.9.163

### Changed
- **The release page's genre lookup no longer waits on MusicBrainz first.** When a release opens and nothing already known has a genre for it, the page used to ask MusicBrainz directly — one request per page you open, and unless you run your own MusicBrainz mirror those are limited to roughly one a second, shared with everything else the plugin needs from them. It now asks the LMS-community metadata service first, which isn't limited and answers in a fraction of the time. It returns MusicBrainz's own genres, so nothing about what you see changes — only how long it takes to arrive. MusicBrainz is still asked whenever that service doesn't know the album or can't be reached, so a release too new for it is no worse off than before.
- **Albums that arrive without a MusicBrainz ID can now show genres too.** Some albums in the Trending lists are assembled from listens that were never matched to MusicBrainz, so the page had nothing to look a genre up with and simply showed none. The new lookup works from the artist and album name, so those pages can fill in as well.

### Internal
- New `API::getAlbumGenresHosted`, going through the same single `_hostedGet` helper as every other hosted call (so it carries the required `X-LMS-Plugin-ID` header and inherits the auth slot). Cached `lbf:hgenres:1:` — 30d for an answer, 1d for an empty one, so a release the service doesn't know yet is rechecked rather than pinned. Capped at 5 genres, matching `_parseGenres`.
- `?mbid=` is **ignored** on `/album/<title>/<artist>/genres` — verified live: a deliberately bogus mbid returns the identical list. The route is name-keyed and cannot be disambiguated, so our release-group MBID is deliberately not sent.
- **No opt-out pref, and none will be added** (decided this session). Every hosted tier is unconditional; the fallback behind each one is what makes that safe, not a setting. The suggestion to add one in `docs/hosted-lms-community-api.md` is struck.
- **No cache bumps.** `lbf:hgenres:` is new and nothing existing changed shape; bumping the match/decision caches would force a full re-resolve of every album and playlist for no benefit.
- `tools/t_genrefill.pl` 34 → 55 assertions (new section 6: path argument order, the absent mbid, per-segment encoding, the cap, an empty answer reaching the caller as an empty list rather than a miss, the short TTL on empty, the cached-empty short-circuit, outage behaviour, and the tier ordering at the call site). Anti-tested three ways — call site reverted to MusicBrainz-only = 3 red, album/artist transposed = 2 red, empty answers cached long = 1 red.
- Re-measured ListenBrainz's bulk metadata endpoint while checking the above: **1572 release groups in 8.06s**, worst batch 0.95s, and the limit is **30 requests per 10 seconds** (`X-RateLimit-Limit`/`Reset-In`) — a 429 on the 31st batch. The genre path stays well inside it (renders 3 batches, the warm 12, both sequential). The old 125s was slow service, not throttling: `x-ratelimit-remaining` sat at 26-29/30 throughout.

## 0.9.162

### Added
- **Genre labels on release rows, and a genre filter.** Every release in New Releases for You and All Releases can now show its genre next to the type — "Album · Electronic", or "Album · Funk (funk rock, funk soul)" where the release has sub-genres of its own. A new **Genres** row in each list's Options section opens a tick-list of the genres actually present in what you're looking at, so you can narrow a week down to the two or three kinds of music you're interested in. The list shows the busiest genres first, and the row tells you how many releases your selection will show before you apply it.
- **This has been built for a while and switched off.** It needed a genre for every release in a feed, and the only source available without running your own MusicBrainz mirror was answering a batch of 50 releases in anything from a quarter of a second to twenty-four seconds — one full feed took over two minutes. Shipping that as a default would have made browsing a lottery for the sake of a label. That service has since been fixed: the same feed now fills in **under three seconds**, so it is on by default for everyone.
- **A "Genre labels" setting** for people who want it otherwise: use a local MusicBrainz mirror when there is one (the default, and the fastest), always use ListenBrainz, or never look genres up at all.
- **Similar artists for the radio without a Last.fm key.** The ListenBrainz Radio mixer falls back to similar artists when ListenBrainz itself has none for what you're playing. That fallback needed a free Last.fm API key, so for most people it simply never happened and the radio dropped to generic recommendations. It now tries the LMS-community metadata service first, which needs no key at all — and because those results arrive with MusicBrainz IDs already attached, the radio starts playing without the long pause it used to take while it looked each artist up one at a time.
- **A "LMS-community API" row in the Connection Check**, so the new service shows up alongside the others. If it can't be reached the plugin falls back to MusicBrainz, so that row reporting a problem means "slower than it should be", not "broken" — and it says so.

### Changed
- **Artist name lookups are much faster for most people.** Turning an artist name into a MusicBrainz ID is what the radio does for every seed and every similar artist, and against public MusicBrainz it is limited to roughly one lookup per second — so a radio station could spend half a minute resolving names before playing anything. Those lookups now go to the LMS-community service first, which isn't throttled. It is only trusted when the name it returns matches the name asked for, and anything else falls back to the previous MusicBrainz search unchanged, so nothing is resolved less carefully than before.
- **The release detail page and the list rows can no longer disagree about genre.** Both now read the same data. The detail page shows the full, specific genres; the lists roll them up to one top-level family, because scanning a week wants "Electronic" rather than four sub-genres per row.
- **Browsing never waits for genres.** Lists always draw immediately with whatever is already known and fill the rest in behind, and a nightly pass keeps the cache warm — so a genre lookup can never hold a page open.

### Fixed
- **The artist genres shown on a row are no longer polished-up user tags.** Only names MusicBrainz recognises as genres are used, so "seen live", country names and moods can't appear as a genre.

### Internal
- Cache keys bumped: `lbf:rgmeta:1:` → `:2:` (entries gained genres) and `lbf:artistmbid:1:` → `:2:` (name resolution can now accept answers the old scored search rejected). Deliberately **not** bumped: `lbf:stream:`, `lbf:track:`, `lbf:pl:resolved:` — the matcher is untouched, so bumping them would force a full re-resolve of every album and playlist for no benefit; and `lbf:bcmatch:`, which is never bumped.
- New test suite `tools/t_genrefill.pl` (34 assertions) covering the lookup-mode default, the genre-tag quality gate and ordering, the bulk-metadata merge, and the name-resolution accept gate — including the case where an unknown artist echoes the query name back with an empty ID. Anti-tested by re-parking the feature and by removing the gate.
- Every hosted-API call goes through one request helper that sends the required `X-LMS-Plugin-ID` header and has a slot for authentication if it is added later.
