...
Then rebuild nothing else — these are docs, not part of the plugin zip.
"""
...
ZIP_NAME = "ListenBrainzFreshReleases.zip"
GITHUB_URL = "https://github.com/SimonArnold002/LMS-ListenBrainz-New-Releases"
# The version badge is read live from install.xml (single source of truth) so a
# regen always reflects the current release; the rest are static.
STATIC_BADGES = ["LMS 9.0.0+", "Material Skin", "Powered by ListenBrainz"]


def read_version(root):
    """Current plugin version from install.xml (the single source of truth)."""
    try:
        with open(os.path.join(root, "ListenBrainzFreshReleases", "install.xml"), encoding="utf-8") as f:
...
<title>ListenBrainz Fresh Releases — LMS Plugin</title>
</head>
<body>
<p>Redirecting to <a href="README.html">the ListenBrainz Fresh Releases page</a>…</p>
</body>
</html>
"""
